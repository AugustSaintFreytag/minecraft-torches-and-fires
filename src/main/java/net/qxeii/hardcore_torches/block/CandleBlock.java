...
public class CandleBlock extends AbstractCandleBlock implements LightableBlock, Waterloggable {
	...
	public CandleBlock(Settings settings, IntSupplier maxFuel, boolean isLit) {
		super(settings, maxFuel, isLit);
		this.setDefaultState(
				(BlockState) ((BlockState) ((BlockState) ((BlockState) this.stateManager.getDefaultState()))
						.with(CANDLES, 1).with(LIT, false)).with(WATERLOGGED, false));
	}
	...
	public boolean canReplace(BlockState state, ItemPlacementContext context) {
		return !context.shouldCancelInteraction() && context.getStack().getItem() == this.asItem()
				&& (Integer) state.get(CANDLES) < 4 ? true : super.canReplace(state, context);
	}
	...
	public boolean tryFillWithFluid(WorldAccess world, BlockPos pos, BlockState state, FluidState fluidState) {
		if (!(Boolean) state.get(WATERLOGGED) && fluidState.getFluid() == Fluids.WATER) {
			...
			if ((Boolean) state.get(LIT)) {
				extinguish((PlayerEntity) null, blockState, (World) world, pos);
			} else {
	...
	static {
		...
		CANDLES_TO_PARTICLE_OFFSETS = (Int2ObjectMap) Util.make(() -> {
			...
			int2ObjectMap.put(3, ImmutableList.of(new Vec3d(0.5D, 0.313D, 0.625D), new Vec3d(0.375D, 0.44D, 0.5D),
					new Vec3d(0.56D, 0.5D, 0.44D)));
			int2ObjectMap.put(4, ImmutableList.of(new Vec3d(0.44D, 0.313D, 0.56D), new Vec3d(0.625D, 0.44D, 0.56D),
					new Vec3d(0.375D, 0.44D, 0.375D), new Vec3d(0.56D, 0.5D, 0.375D)));
